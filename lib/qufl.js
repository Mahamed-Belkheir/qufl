...
class Qufl {
    constructor({ algorithm = "HS256", cookieKey = "QUFLTOKENKEY", passError = false, secret = "", tokenTimeout = "1h", store = undefined } = {}) {
        this.extractors = {
            bearer: (req) => {
        ...
        this.auth = (options = {}) => {
            var _a;
            let extractor;
            (_a = options.tokenType) !== null && _a !== void 0 ? _a : (options.tokenType = "token");
        ...
        this.store = new store_1.StoreFacade(options.store);
    ...
    signToken(data) {
        return __awaiter(this, void 0, void 0, function* () {
            let token = jsonwebtoken_1.default.sign(Object.assign(Object.assign({}, data), { type: Token }), this.options.secret, {
                expiresIn: this.options.tokenTimeout,
                algorithm: this.options.algorithm
            });
            let refresh = jsonwebtoken_1.default.sign(Object.assign(Object.assign({}, data), { type: Refresh }), this.options.secret, {
                algorithm: this.options.algorithm
            });
            yield this.store.set(data.aud + ":" + data.sub, true);
            return { token, refresh };
        });
    }
    refreshToken(refreshToken) {
        return __awaiter(this, void 0, void 0, function* () {
            let result = yield this.store.get(refreshToken.aud + ":" + refreshToken.sub);
            if (!result) {
    ...
    removeToken(token) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.store.destroy(token.aud + ":" + token.sub);
        });
    }
